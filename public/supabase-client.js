...
export async function billingAction(action){if(!session)throw new Error("Sign in to manage billing.");const response=await fetch(`/api/billing/${action}`,{method:"POST",headers:{"x-stockscope-session":session.access_token}}),text=await response.text();let data;try{data=text?JSON.parse(text):{}}catch{throw new Error(text||"Billing request failed.")}if(!response.ok)throw new Error(data.error||"Billing request failed.");return data}
