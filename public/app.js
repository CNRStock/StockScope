...
function bindDiscoveryResearch(){
  $$('[data-research-match]').forEach(button=>button.addEventListener('click',()=>{const candidate={ticker:button.dataset.ticker,type:button.dataset.type,name:button.dataset.name||button.dataset.ticker},anchor={ticker:current.ticker,type:current.assetType,name:current.ticker};window.dispatchEvent(new CustomEvent('assetseek:compare-candidate',{detail:{anchor,candidate}}))}));
}
