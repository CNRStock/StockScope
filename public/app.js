...
renderAccount();
async function renderSettings(){showView("settings");$("#settingsEmail").textContent=currentSession().user.email;const upgrade=$("#upgradePro"),manage=$("#manageBilling"),message=$("#settingsMessage"),detail=$("#settingsPlanDetail");try{const [profile,enabled]=await Promise.all([loadProfile(),billingAvailable()]),isPro=profile?.plan==="pro",dateValue=profile?.cancel_at||profile?.current_period_end,date=dateValue?new Intl.DateTimeFormat("en-GB",{dateStyle:"long"}).format(new Date(dateValue)):null;$("#settingsPlan").textContent=(profile?.plan||"free").toUpperCase();upgrade.classList.toggle("hidden",isPro);manage.classList.toggle("hidden",!isPro);upgrade.disabled=!enabled;if(profile?.cancel_at_period_end&&date)detail.textContent=`Pro remains active until ${date}, then returns to Free.`;else if(profile?.subscription_status==="payment_failed"||profile?.subscription_status==="past_due")detail.textContent="A payment failed. Update your payment method in Manage billing to keep Pro access.";else detail.textContent=isPro?(date?`Active subscription. Next billing date: ${date}.`:"Active Pro subscription."):"Free accounts receive 5 calculations and 3 similarity searches per day.";if(!enabled&&!isPro)message.textContent="Pro checkout is being prepared and is not open yet."}catch{$("#settingsPlan").textContent="FREE"}}
...
$("#run").addEventListener("click",async()=>{
  ...
  try{
    const usage=await consumeUsage("calculation");if(!usage.allowed)throw new Error("Daily free calculation limit reached. Pro access will remove this limit.");
...
$("#findSimilar").addEventListener("click",async()=>{
  if(!current)return;
  const usage=await consumeUsage("similarity");if(!usage.allowed){alert("Daily free similarity limit reached. Pro access will remove this limit.");return}
  showView("similar");
...
async function renderSaved(){
  if(currentSession()){
    const el=$("#savedList");
    try{
      const rows=await loadResearch();if(!rows.length){el.innerHTML='<div class="glass empty">No account research saved yet.</div>';return}
